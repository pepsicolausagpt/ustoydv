                      ...
                      return (
                        <div key={row.id || ri} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px', padding: '12px', background: row.enabled === false ? '#fff1f2' : '#fafafa', borderRadius: '8px', border: '1px solid #e2e8f0', opacity: row.enabled === false ? 0.7 : 1 }}>
                                ...
                                {row.rows.map((sub, si) => {
                                  ...
                                  return (
                                    <tr key={si} style={{ opacity: sub.enabled === false ? 0.45 : 1, background: sub.enabled === false ? '#fff1f2' : 'transparent' }}>
                                      ...
                                      <td style={{ border: '1px solid #cbd5e1', padding: '2px', textAlign: 'center' }}>
                                        <input value={sub.n} onChange={(e) => updateComplexSubRow(secIdx, ri, si, 'n', e.target.value)} style={{ width: '24px', textAlign: 'center', border: 'none', background: 'transparent', fontSize: '11px' }} />
                                      </td>
                                      <td style={{ border: '1px solid #cbd5e1', padding: '0' }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                            <button onClick={() => addComplexSubRow(secIdx, ri)} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer', border: '1px dashed #ccc', borderRadius: '4px', background: 'none' }}>+ Добавить строку</button>
                            <button onClick={() => addComplexSubGroup(secIdx, ri)} style={{ padding: '4px 12px', fontSize: '11px', cursor: 'pointer', border: '1px dashed #ccc', borderRadius: '4px', background: 'none' }}>+ Добавить заголовок (4 м. / 6 м.)</button>
                          </div>
                        </div>
                      );
                    }
                      ...
                      <div key={row.id || ri} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px', padding: '12px', background: row.enabled === false ? '#fff1f2' : '#fafafa', borderRadius: '8px', border: '1px solid #e2e8f0', opacity: row.enabled === false ? 0.7 : 1 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingLeft: '24px' }}>
                          ...
                          <input type="file" accept="image/*" onChange={async (e) => {
                            ...
                            setLoading(true);
                            try {
                              const publicUrl = await uploadImageToGithub(e.target.files[0]);
                              updateSimpleRowField(secIdx, ri, 'img', publicUrl);
